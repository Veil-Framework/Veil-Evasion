...
int VoIWYKYKccMMTqo(ARCHIVE_STATUS *bpBERTfwO, char const * OphxlTNEOPBo, char  const * DNhIlpSOZom);
int pbtRZQU(ARCHIVE_STATUS *PLblUEHNuotdGG[]);
int BHbyyDe(ARCHIVE_STATUS *quemriBfexWjk, int VOqPUIcERTO, char *TkTEDtbpIRVFPZe[]);
int callSimpleEntryPoint(char *MHeyKVlG, int *jZRsRYAu);
void aVWIXVZ(ARCHIVE_STATUS *goXVvzHK);
int aJYNvnshnpHSQgm(ARCHIVE_STATUS *yqoidVu);
void WowgHWrzEQiPD(void);
int rDvZnGjxrftwm(ARCHIVE_STATUS *stKdZLrlGzw, char const * BRoeOiWNEmuyU, char const * hYroYBzgubTWM);
int ndqRJbtnndS(ARCHIVE_STATUS *gApXgX);
int attachPython(ARCHIVE_STATUS *aygAAMeuQ, int *IvnxzTK);
int DOaCRqBZKiYumv(ARCHIVE_STATUS *erzcQvqIOzoWX);
int kNTpLDYoKCw(ARCHIVE_STATUS *AOBjOLfWumN, int oTxkyrOVZ, char *MrGgZJenVNISXbE[]);
int HdPmDvVVxEsKom(ARCHIVE_STATUS *xlJbAYweIsFn);
int ZoJMtJHFzYkkW(ARCHIVE_STATUS *ZzucRMxGbLQ);
int HZWOOvAo(ARCHIVE_STATUS *wgKGYGTIU);
TOC *getFirstTocEntry(ARCHIVE_STATUS *PEBEljSVwXUnN);
TOC *getNextTocEntry(ARCHIVE_STATUS *hYAqpcumdhbULnT, TOC *vHsApfv);
void JlmqoR(const char *kfEisWzYfkrBtL);
#endif
